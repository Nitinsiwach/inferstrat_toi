...
master.train.dt <- train.dt
innov_list <- c("Full.Page.Jacket", "GLUED.HALF.FLAP", "Full.Page.with.gluing", "halfFlapwithGlue")
...
modelflow <- function(x){
counter = 1
lives = 0
while(counter){
#print(length(names(train.dt)))
ctrl= trainControl(method= "none")
ret= buildGBM(depth = 4, trees = 250, nodesize = 10, model_data = train.dt)
...
#print(length(vars.i.want))
train.dt= train.dt[, c(vars.i.want, 'targ'), with=F]   #and now go back and rerun the model
#print(length(train.dt))

if (length(vartable[importance < 0.5]$varname) == 0) {
  if (!(x %in% colnames(train.dt))) {
    train.dt[,x] <- master.train.dt[,x,with=F]
    colnames(train.dt)[colnames(train.dt) == x] <- x
...
print(paste('finalAdamModelavgspeed17', x,'.RDS', sep = ""))
saveRDS(mymodel, paste('finalAdamModelavgspeed17', x,'.RDS', sep = ""))
saveRDS(vartable, paste('finalModelVarImportancesavgspeed17', x ,'.RDS', sep = ""))
}
mapply(modelflow, innov_list)
